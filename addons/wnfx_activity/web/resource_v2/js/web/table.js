define(['jquery'], function($) {
	...
	$(document).on('change', '.page-table-header input[type="checkbox"]', function(e) {
		...
		$('tfoot [type="checkbox"]', $table).prop('checked', $checked);
		$('tbody [type="checkbox"]', $table).prop('checked', $checked);
		$('.tables [type="checkbox"]').prop('checked', $checked);
		$('.table-footer [type="checkbox"]').prop('checked', $checked);
	});
	$(document).on('change', '.table-footer [type="checkbox"]', function(e) {
		e && e.preventDefault();
	...
	var div = $(".table-responsive"),
		...
		if (div.prop('nodeName')=='DIV') checkboxs=$('.tables .table-row:nth-child(2) [type="checkbox"]', div);
	if (div.length > 0) {
		...
		var get_selecteds = function() {
				...
				if (div.prop('nodeName')=='DIV')
					selected_checkboxs=$('.tables .table-row:nth-child(2) [type="checkbox"]:checked', div);
		...
		batch.on("click", function() {
			...
			var selecteds = get_selecteds();
			if (div.prop('nodeName')=='DIV') chks=$('.tables .table-row:nth-child(2) [type="checkbox"]:checked', div);
			var submit = function() {
					if ($this.data('toggle') != 'batch-modal')
							...
							if ($this.data('toggle') == 'batch-remove') {
								var deferred = $.Deferred(),
									removeHandler = function(def) {
										var c = 0;
										return chks.parents("tr,.tables").fadeOut(function() {
											$(this).remove(), c++, chks.length == c && def.resolve()
										}), def
									};								
								$.when(removeHandler(deferred)).done(function() {
									if (div.prop('nodeName')=='TABLE')
										batch.attr("disabled", 0 == $('tbody tr td:first-child [type="checkbox"]:checked', div).length), 0 == $("table tbody tr", div).length
									else
										batch.attr("disabled", 0 == $('.tables .table-row:nth-child(2) [type="checkbox"]:checked', div).length), 0 == $(".tables", div).length
								})
							} else {
								if (div.prop('nodeName')=='TABLE')
									batch.attr("disabled", 0 == $('table tbody tr td:first-child [type="checkbox"]:checked', div).length);
								else
									batch.attr("disabled", 0 == $('.tables .table-row:nth-child(2) [type="checkbox"]:checkedd', div).length);
		...
		}), $(document).on("change", '.table-responsive tfoot td:first [type="checkbox"],.table-responsive .table-footer [type="checkbox"]', function(e) {
			e && e.preventDefault();
		...
		}), $(document).on("change", '.table-responsive .tables .table-row:nth-child(2) [type="checkbox"]', function(e) {
			e && e.preventDefault();
			var t = $(e.target).closest(".table-responsive"),
				checked = $(e.target).is(":checked"),
				chk = $('.tables .table-row:nth-child(2) [type="checkbox"]:checked', t);
			$('.page-table-header input[type="checkbox"]').prop("checked", checked && chk.length == checkboxs.length);
			$('.table-footer div:first [type="checkbox"]', t).prop("checked", checked && chk.length == checkboxs.length);
			if (chk.length > 0) {
				batch.add(trigger).removeAttr("disabled")
			} else {
				batch.add(trigger).attr("disabled", "disabled")
			...
			var submit = function() {
					...
					$.post(url).done(function(data) {
						data = eval("(" + data + ")");
						if (data.status == 1) {
							var tr = obj.parents("tr,.tables");
							tr.css({
								"background-color": "#dff0d8"
							}).find("td").css({
								"background-color": "#dff0d8"
							}), tr.fadeOut(function() {
								tr.remove(), 0 == $(obj.closest('tbody')).find('tr').length
							})
						} else {
