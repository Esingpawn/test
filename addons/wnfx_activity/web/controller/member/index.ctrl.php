...
if ($_W['op'] == 'setblack') {
	...
	foreach ($uids as $uid) {		
		$member = pdo_fetch('select * from ' . tablename('fx_member') . (' WHERE uid=' . $uid . ' AND uniacid=') . $_W['uniacid']);
		if(!empty($member)){
			pdo_update('fx_member', array('isblack' => $black), array('id' => $member['id']));
		}else{
			pdo_insert ('fx_member', array (
				'uid' => $uid,
				'uniacid' => $_W['uniacid'],
				'isblack' => $black
			));
