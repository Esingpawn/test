...
{  
    ...
    {  
		...
		*/
        ...
        {  
			...
			$column_len = $clOffset==0 ? 20 : ($clOffset==12 || $clOffset==13?35:($clOffset==3 || $clOffset==5 || $clOffset==6 || $clOffset==7?9:15));
	...
	public static function saveExcelFile($objExcel, $fileName, $path)  
    {
		...
		$outputFileName = mb_convert_encoding($outputFileName,"GBK","utf-8");
	 ...
	 * @param string $path 输出路径
     */ 
	public static function getZipFile($temp_folder, $fileName, $base_path)  
    {
		// 下载文件
		setlocale(LC_ALL, 'zh_CN.GBK');
		$path = $base_path . $temp_folder;
		$save_filepath = $path . "/" . $fileName . '.zip';
		$save_filepath = mb_convert_encoding($save_filepath,"GBK","utf-8");
		echo $save_filepath;exit;
		$zip = new ZipArchive;
		...
		if(is_dir($path))
		{
			...
			while(($file = readdir($handler)) !== false )
			{
				if($file != "." && $file != "..")
				{
					if(is_dir($path."/".$file))
					{
						self::addFileToZip($path."/".$file, $zip, $base_path);
					}
					else
					{
