...
load()->func('tpl');
class Wnfx_activityModule extends WeModule {
	...
	public function settingsDisplay($settings) {
		...
		if(checksubmit()) {
			load()->func('file');
	        $r = mkdirs(IA_ROOT . '/attachment/wnfx_activity/cert/'. $_W['uniacid']);
			if(!empty($_GPC['cert'])) {
	            $ret = file_put_contents(IA_ROOT . '/attachment/wnfx_activity/cert/'.$_W['uniacid'].'/apiclient_cert.pem', trim($_GPC['cert']));
	            $r = $r && $ret;
	        }
	        if(!empty($_GPC['key'])) {
	            $ret = file_put_contents(IA_ROOT . '/attachment/wnfx_activity/cert/'.$_W['uniacid'].'/apiclient_key.pem', trim($_GPC['key']));
	            $r = $r && $ret;
	        }
			if(!empty($_GPC['rootca'])) {
	            $ret = file_put_contents(IA_ROOT . '/attachment/wnfx_activity/cert/'.$_W['uniacid'].'/rootca.pem', trim($_GPC['rootca']));
	            $r = $r && $ret;
	        }
			if(!$r) {
