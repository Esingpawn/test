...
$order = pdo_get('fx_activity_records', array('orderno' => $log['tid'], 'uniacid' => $_W['uniacid']));

if ($order['status']==5) die(json_encode(array('errno'=>1, 'message'=>"订单已取消，请重新下单！")));

if ($order['optionid'] && $activity['hasoption']){//规格选项
...
if ($gnum > 0){
	$fp = fopen(IA_ROOT . '/addons/' . IN_MODULE . '/data/files/pay.lock', "w+");
	if(flock($fp, LOCK_EX)){
		$joinnum = model_records::getJoinNum($activity['id'], $order['optionid'], $activity['falsedata']['num'], true);
		if($joinnum >= $gnum) {
