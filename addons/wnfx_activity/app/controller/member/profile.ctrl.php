...
$_W['op']  = !empty($_W['op']) ? $_W['op'] : 'display';
...
if($_W['op'] =='display'){
	...
	include fx_template('member/profile');
	exit;
}
//关注的主办方
if($_W['op'] =='list'){
	...
	if ($_W['isajax']) {
		...
		foreach ($fansData[0] as &$s) {
			...
			if (!$s['merchantid']){
				...
				if ($s['merchantid']){
					$merchant = pdo_fetch("SELECT * FROM " . tablename ('fx_merchant') . " WHERE id=" . $s['merchantid']);
				}else{
				...
				if (!empty($merchant)){
					...
					$member = m('member')->getMember($s['muid']);
					$condition.= " and merchantid=-1";
				}
				$s['name'] = $merchant['name'];
				$s['logo'] = tomedia($merchant['logo']);
				$s['detail'] = empty($merchant['detail']) ? 'Ta太懒了，什么也没有留下' : $merchant['detail'];
			}
		...
		include fx_template('member/profile_list');
	}
	exit;
}
//读取当前主办方活动
if($_W['op'] =='activity' && $_W['isajax']){
	$pindex = max(1, intval($_GPC['page']));
	$psize = $_GPC['pagesize']?$_GPC['pagesize']:15;
	$where = array(
		'show' => 1,
	...
	$where['sql'][] = "(merchantid=$merchantid OR atype=3)";
	...
	foreach ($activityData[0] as &$s) {
		...
		if($s['hasoption']==1){
			$aprice['max'] = pdo_fetchcolumn("SELECT max(marketprice) as aprice FROM " . tablename('fx_spec_option') . " WHERE costprice > 0 and activityid = " .$s['id']);
			$aprice['min'] = pdo_fetchcolumn("SELECT min(marketprice) as aprice FROM " . tablename('fx_spec_option') . " WHERE costprice > 0 and activityid = " .$s['id']);
