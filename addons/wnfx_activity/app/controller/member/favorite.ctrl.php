...
if($_W['op'] =='display'){
	include fx_template('member/favorite_list');
	exit;
}
...
if ($_W['op'] == 'ajax') {
	...
	foreach ($favorData[0] as &$s) {
		...
		if (!empty($activity)){
			...
			$s['thumb'] = tomedia($activity['thumb']);
			...
			//读取规格名额
			if($s['hasoption']==1){
				$aprice['max'] = pdo_fetchcolumn("SELECT max(marketprice) as aprice FROM " . tablename('fx_spec_option') . " WHERE costprice > 0 and activityid = " .$s['activityid']);
				$aprice['min'] = pdo_fetchcolumn("SELECT min(marketprice) as aprice FROM " . tablename('fx_spec_option') . " WHERE costprice > 0 and activityid = " .$s['activityid']);
				$s['minaprice'] = !empty($aprice['min']) && $aprice['max'] > $aprice['min']?$aprice['min']:0;
				//读取规格总名额，总虚拟人数
				$opt['stock'] = pdo_fetchcolumn("SELECT SUM(stock) FROM " . tablename('fx_spec_option') . " WHERE activityid = " .$s['activityid']);
				$opt['nolimit'] = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('fx_spec_option') . " WHERE stock = 0 and activityid = ".$s['activityid']);
...
if($_W['op'] =='set'){
	...
	$id = intval($_GPC['fid']);
	$aid = intval($_GPC['id'])?intval($_GPC['id']):intval($_GPC['activityid']);
	$where = array('id' => $id, 'uniacid' => $_W['uniacid']);
	$activity = model_activity::getSingleActivity($aid, 'id');
	if (!empty($activity)){
