...
if($_W['op'] =='display'){
	...
	include fx_template('member/merch');
	exit;
}
//收入管理
if($_W['op'] =='income'){
	$pagetitle = '我的收入';
	$account =  pdo_fetch("SELECT * FROM ".tablename('fx_merchant_account')." WHERE $uniacid and merchantid=" . MERCHANTID);
	$w_money  = pdo_fetchcolumn("select SUM(money) from".tablename('fx_merchant_record')." where $uniacid and status<>3 and merchantid=" . MERCHANTID);
	$y_money  = pdo_fetchcolumn("select SUM(money) from".tablename('fx_merchant_record')." where $uniacid and merchantid=" . MERCHANTID);
	$no_hexiao = pdo_fetchcolumn("select SUM(price) from".tablename('fx_activity_records')." where $uniacid and (paytype='wechat' or paytype='alipay') and status in(1) and merchantid=" . MERCHANTID);
	$merchant = model_merchant::getSingleMerchant(MERCHANTID, '*');
	$merchant['amount'] = $account['amount'];
	$merchant['w_money'] = number_format($w_money,2);//待处理的提现金额
	$merchant['y_money'] = number_format($y_money,2);//已处理的提现金额
	$merchant['no_hexiao'] = number_format($no_hexiao,2);//待核销
	...
	include fx_template('member/merch_income');
	exit;
}
//读取明细
if($_W['op'] =='income_list'){
	$pagetitle = '明细';
	if ($_W['isajax']) {
		$pindex = max(1, intval($_GPC['page']));
		$psize = $_GPC['pagesize']?$_GPC['pagesize']:15;
		$where['merchantid'] = MERCHANTID;
		$moneyRecordData = Util::getNumData('id,money,createtime,type,detail', 'fx_merchant_money_record', $where, 'createtime desc', $pindex, $psize, 1);
		$data = array();
	...
	include fx_template('member/merch_income_list');
	exit;
}
//提现操作
if($_W['op'] =='cash'){
	...
	$merchant = model_merchant::getSingleMerchant(MERCHANTID, '*');
	if (!$merchant['iscert'] && $_W['_config']['certswitch']) fx_message('访问失败', app_url('member/merch'), 'warning','你还没有认证，不能进行提现操作！');
	...
	include fx_template('member/merch_cash');
	exit;
}
//提现记录
if($_W['op'] =='cash_list'){
	...
	include fx_template('member/merch_cash_list');
	exit;
}
//待确认记录
if($_W['op'] =='confirm'){
	...
	include fx_template('member/merch_confirm');
	exit;
}
...
if($_W['op'] =='fans'){
	...
	if ($_W['isajax']) {
		...
		foreach ($fansData[0] as &$s) {
			...
			$member = m('member')->getMember($s['uid']);
		...
		include fx_template('member/merch_fans');
	}
	exit;
}
if($_W['op'] == 'help'){
	...
	include fx_template('member/help_list');
	exit;
}
if($_W['op'] == 'hexiao'){
	$pagetitle = '核销管理';
	...
	include fx_template('member/merch_hexiao');
}

//更新主办方信息
if ($_W['op'] == 'update' && $_W['isajax']) {
	$type = $_GPC['type'];
	...
	if (ADMIN){
		$pars = array('module' => IN_MODULE, 'uniacid' => $_W['uniacid']);
		$sys = array();
		$sys['settings'] = iserializer($settings);
		$result = pdo_update('uni_account_modules', $sys, $pars);
		cache_build_module_info(IN_MODULE);
	}else{
		if (!empty($data['logo'])){
	...
	if ($result) {
		$merchant = model_merchant::getSingleMerchant(MERCHANTID, '*');
		die(json_encode(array('message' => '更新资料成功！','type'=>'success','merchant'=>$merchant)));
	}else{
		die(json_encode(array('message' => '相同信息不做修改！','type'=>'error')));
	}
	exit;
}
if($_W['op'] == 'mcert'){
	...
	if (!empty($mcert)){
		...
		foreach ((array)$mcert['detail']['thumb'] as $i=>$img) {
			$mcert['detail']['pic'][$i] = tomedia($img);
		}
	}
