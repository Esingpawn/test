$(document).ready(function() {
	...
	$(".play-video").on("click", function() {
		...
		var datasrc = $(this).attr("data-src"),datahide = $(this).attr("data-hide")?' hide':'',src='';
		var  dangqian="";
		...
		var str = ["<div class='video-box " + vClass + "' id='" + videoid + "box'>",
			...
			"<div class='video-close'>退出播放</div>",
...
//			$(".video-bofang.f-czspjuzhong").removeClass("video-hide");
		...
		$(".video-video, .video-video~.video-bofang, .video-loading").on({
			...
			touchend: function(e){	
				//通过滑动的角度判断触摸的方向
				if((angle<45 && angle>=-45) || (angle<=180 && angle>=135) || (angle>=-180 && angle<-135 )){
					//console.log('左右滑动');
					if (nx<80 && nx>-80) return false;
					setTimeout(function() {
						videobox.remove();
						$(".video-bofang").find("i").removeClass("i-zhanting").addClass("i-bofang");
					}, 100);
