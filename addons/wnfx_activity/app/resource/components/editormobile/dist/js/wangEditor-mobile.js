					...
					reader.onload = function(e){
						...
						if (window.URL && window.URL.createObjectURL) {
							...
							var fn = function () {
								...
								'	<img src="' + prevImgSrc + '" style="opacity:0.2;max-width:100%;"/><br>' + 
								'</div>');
							};
							self.$focusElem.after('<br><span style="color:rgb(127, 127, 127);font-size: 12px;">&nbsp;<span>');
							// 执行预览图命令
							self.customCommand(false, fn, e);
							placeCaretAtEnd(self.$focusElem[0].parentElement);
							...
							$focusElem.after(
								...
								'	<img src="' + prevImgSrc + '" style="max-width:100%;"/><br>' + 
								'</div>'
							);
						}
			            ...
			            xhr.onload = function () {
			            	...
			            	if (resultSrc.indexOf('error|') === 0) {
			            		...
			            		log('准备下载该图片...');
								//更新附件信息id
			            		//$.post('../app/index.php?i=' + util.querystring("i") + '&c=entry&m=wnfx_activity&do=utility&ac=file&op=upload',{attachid:getData.id});
			            		// 下载图片，下载完成后赋值到编辑器中
						...
						function compress(img) {
							...
							img.onload = function(){
								...
								var quality = 0.98;  // 默认图片质量为0.7
								var pixel = 837000;  // 如果图片大于四百万像素，计算压缩比并将大小压至400万以下
								...
								//进行最小压缩
								var ndata = canvas.toDataURL(fileType, quality);
...
window.___E_mod(function (E, $) {
	...
	E.fn.saveSelection = function (range) {
		...
		if (_parentElem && (txt.contains(_parentElem) || txt === _parentElem) ) {
			...
			if (self.config.menuTap) {
				wrapRange.selectNodeContents(_parentElem);
				self.config.menuTap = false;
			}
