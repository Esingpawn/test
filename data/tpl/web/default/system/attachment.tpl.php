...
<div class="clearfix" id="js-global-attachment" ng-controller="globalAttachmentCtrl" ng-cloak>
	...
	<div class="form-files-box">
		...
		<div class="form-files we7-margin-bottom">
			<div class="form-file header">附件展示设置</div>
			<div class="form-file">
				<div class="form-label">展示设置</div>
				<div class="form-value">开启后，平台账号下的附件会根据角色进行展示，即非当前角色上传的附件不展示，(副)创始人、主管理员除外</div>
				<div class="form-edit">
					<div ng-class="!upload.attachment_by_uid ? 'switch' : 'switch switchOn'"  ng-click="saveSetting(upload.attachment_by_uid ? 0 : 1, 'attachment_by_uid')"></div>
				</div>
			</div>
		</div>
	</div>
	...
	<label class="col-xs-12 col-sm-3 col-md-2 control-label">bucket所在区域</label>
		<div class="col-sm-9 col-xs-12">
			<select class="form-control" name="cos[local]">
				<?php  if(is_array($cos_bucket_area)) { foreach($cos_bucket_area as $bucket_key => $bucket_name) { ?>
				<option value="<?php  echo $bucket_key;?>" <?php  if($remote['cos']['local'] == $bucket_key) { ?>selected<?php  } ?>><?php  echo $bucket_name;?></option>
				<?php  } } ?>
			</select>
