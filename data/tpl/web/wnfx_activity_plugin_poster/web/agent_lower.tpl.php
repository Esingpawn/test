...
<?php  if(strpos($op,'display') !== false) { ?>
<div class="panel">
    <div class="panel-body">
        <div style="height:80px;width:90px;float:left;">
            <img src="<?php  echo $member['avatar'];?>" style="width:80px;height:80px;border:1px solid #ccc;padding:1px">
        </div>
        <div style="float:left;height:80px;overflow:hidden">
            昵称: <?php  echo $member['nickname'];?><br>
            姓名: <?php  echo $member['realname'];?> <br>
            手机号: <?php  echo $member['mobile'];?><br>

            下级分销商: 总共 <span style='color:red'><?php  echo $lower['lowers'];?></span> 人
                <?php  if($set['level']>=1) { ?> 一级: <span style='color:red'><?php  echo $lower['first'];?> </span> 人 <?php  } ?>
                <?php  if($set['level']>=2) { ?> 二级: <span style='color:red'><?php  echo $lower['second'];?></span> 人 <?php  } ?>
                <?php  if($set['level']>=3) { ?> 三级: <span style='color:red'><?php  echo $lower['third'];?></span> 人 <?php  } ?>
		</div>
    </div>
</div>
...
<div class="we7-page-search we7-padding-bottom clearfix">
	...
	<form action="" method="get" class="form-inline" role="form">
        ...
        <input type="hidden" name="op" value="display.lower" />
...
<form method="post" class="we7-form" id="form1">
    <table class="table we7-table table-hover vertical-middle" style="<?php echo substr(IMS_VERSION, 0, 1)>=2?'margin-bottom:35px':''?>">
        ...
        <tr>
            ...
            <td>
            总计：<?php  echo $row['lowers'];?> 人
            <?php  if($set['level']>=1) { ?><br/>一级：<?php  echo $row['first'];?> 人<?php  } ?>
            <?php  if($set['level']>=2) { ?><br/>二级：<?php  echo $row['second'];?> 人<?php  } ?>
            <?php  if($set['level']>=3) { ?><br/>三级：<?php  echo $row['third'];?> 人<?php  } ?></td>
