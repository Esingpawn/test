...
<ul class="nav nav-tabs we7-margin-bottom">
    <li<?php  if($_GPC['status']=='') { ?> class="active"<?php  } ?>>
        <a href="<?php  echo web_url('withdraw', array('op' => 'display','status'=>''))?>">全部记录</a>
    </li>
    <li<?php  if($_GPC['status']=='0') { ?> class="active"<?php  } ?>>
        <a href="<?php  echo web_url('withdraw', array('op' => 'display','status'=>0))?>">待审核</a>
    </li>
    <li<?php  if($_GPC['status']==1) { ?> class="active"<?php  } ?>>
        <a href="<?php  echo web_url('withdraw', array('op' => 'display','status'=>1))?>">待打款</a>
    </li>
    <li<?php  if($_GPC['status']==4) { ?> class="active"<?php  } ?>>
        <a href="<?php  echo web_url('withdraw', array('op' => 'display','status'=>4))?>">打款中</a>
    </li>
    <li<?php  if($_GPC['status']==2) { ?> class="active"<?php  } ?>>
        <a href="<?php  echo web_url('withdraw', array('op' => 'display','status'=>2))?>">已打款</a>
    </li>
    <li<?php  if($_GPC['status']==-1) { ?> class="active"<?php  } ?>>
        <a href="<?php  echo web_url('withdraw', array('op' => 'display','status'=>-1))?>">无效提现</a>
    </li>    
</ul>
	...
	<form action="" method="get" class="form-inline ng-pristine ng-valid" role="form">
		...
		<div class="form-group col-md-9" style="padding-left:0">
            ...
            <div class="input-group col-md-5">
                <input name="search[withdraw_sn]" value="<?php  echo $search['withdraw_sn'];?>" class="form-control" placeholder="提现编号" type="text" style="80%">
...
<form method="post" class="we7-form" id="form1">
    <table class="table we7-table table-hover vertical-middle" style="<?php echo substr(IMS_VERSION, 0, 1)>=2?'margin-bottom:35px':''?>">
        ...
        <tr>
            <td><a href=""><span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;width:78%;display:block;"><?php  echo $row['withdraw_sn'];?></span></a></td>
            ...
            <td>提现到<?php  echo $row['pay_way_name'];?></td>
            <td><?php  echo $row['amounts'];?></td>
            <td><?php  echo $row['created_at'];?></td>
            <td><?php  echo $row['status_name'];?></td>
            <td style="overflow:visible;">
            	<div class="link-group">
                    <a href="<?php  echo web_url('withdraw', array('op' => 'detail','id'=>$row['id']))?>" target="_blank">详情</a>
                </div>
            </td>
        </tr>
        <?php  } } ?>
