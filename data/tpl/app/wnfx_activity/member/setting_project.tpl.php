...
.mui-input-row{background-color:#fff!important;z-index:1}
.group-list .mui-input-row {cursor:move;}
.group-list .mui-input-row label:first-child{width:100%;}
.group-list .mui-input-row .mui-label{width:100%;padding-left:6rem;margin-left:-4.5rem;}
.group-list .mui-input-row .mui-ext-icon,.group-child .mui-input-row .mui-ext-icon{position:absolute;top:50%;z-index:10;width:40px;height:40px;line-height:41.5px;text-align:center;color:#ccc;font-size:18px;-webkit-transform: translateY(-50%);transform: translateY(-50%);}
...
.group-list .mui-input-row.mui-help .mui-help-info{width:4rem;padding:0.5rem 0}
.group-list .mui-input-row.mui-help .mui-help-info .mui-checkbox{font-size:0.55rem;line-height:1.3}
.group-list .mui-input-row.mui-help .mui-help-info span{margin-right:2rem;border:0;color:#ccc}
.group-list .mui-input-row.mui-help .mui-help-info input[type=checkbox]{width:25px;height:25px;right:1.5rem;}
...
<div id="act_spec" class="mui-popover mui-popover-left">
	<div class="mui-popover-header">费用设置
		<a class="mui-pull-right mui-popover-close js-popover-close" data-popover="act_spec"><me class="mui-icon mui-icon-closeempty"></me></a>
	</div>
		...
		<div class="mui-scroll-wrapper-ext">
			<div class="mui-scroll-ext">
                ...
                <div id="specs" class="mui-input-group group-list">
                    <ul class="mui-table-view mui-afterbefore-no" style="margin-top:0;">
                        <li class="mui-table-view-cell mui-media">
                            <a class="mui-navigate-right js-spec-edit" data-id="default" data-popover="spec_edit">
                            ...
                            <input name="activity[gnum]" type="hidden" value="<?php  echo $activity['gnum'];?>" />
                            <input name="activity[aprice]" type="hidden" value="<?php  echo $activity['aprice'];?>" />
                            <input name="activity[costprice]" type="hidden" value="<?php  echo $activity['costprice'];?>" />
                        </li>
                    </ul>
                    <h5 class="mui-content-padded"></h5>
                    <?php  if(count($specs['0'])<=1) { ?>
                    <?php  if(is_array($specs['0'])) { foreach($specs['0'] as $spec) { ?>
                    <?php (!empty($this) && $this instanceof WeModuleSite || 0) ? (include fx_template('activity/spec', TEMPLATE_INCLUDEPATH)) : (include fx_template('activity/spec', TEMPLATE_INCLUDEPATH));?>
                    <?php  } } ?>
                    <?php  } else { ?>
                    <div class="js-specs">
                ...
                <div class="mui-text-primary" style="background:#fff;overflow:hidden">
                    <div class="mui-content-padded mui-text-center js-add-spec"><?php  if(count($specs['0'])<=1) { ?>设置规格<?php  } else { ?>重置规格<?php  } ?></div>
                </div>
				<div class="mui-content-padded">
                	<input type="hidden" id="hasoption" value="<?php echo $activity['hasoption']?$activity['hasoption']:0?>" name="activity[hasoption]" />
                    <button type="button" class="mui-btn mui-btn-orange mui-btn-block js-save" data-type="spec">完成 · 返回</button>
				</div>
			</div>
		</div>
	</div>
            ...
            <div class="mui-scroll">
                ...
                <div class="mui-input-group js-item-option" style="display:none;">
					<div class="mui-input-row">
						<label>数量：</label>
						<input name="aprice" type="text" value="<?php  echo $activity['gnum'];?>" placeholder="不填写无限制" class="js-spec-val" />
					</div>
					<div class="mui-input-row">
						<label>费用：</label>
						<input name="aprice" type="text" value="<?php  echo $activity['aprice'];?>"  placeholder="免费请填0，<?php  echo $_W['_config']['buytitle'];?>后不可更改" class="js-spec-val" />
					</div>
					<div class="mui-input-row"<?php  if(!$_W['plugin']['card']['config']['card_enable']) { ?> style="display:none"<?php  } ?>>
						<label>年卡价：</label>
						<input name="costprice" type="text" value="<?php  echo $activity['costprice'];?>" placeholder="不填写无年卡价" class="js-spec-val" />
					</div>
				</div>
                <div class="mui-content-padded" style="padding-bottom:10px;">
                	<input name="" type="hidden" class="js-itemid" value="" />
                    <button type="button" class="mui-btn mui-btn-orange mui-btn-block js-save" data-type="specEdit">下一步</button>
                </div>
            </div>
		</div>
    </div>
...
<div id="act_form" class="mui-popover mui-popover-action mui-popover-bottom mui-popover-sub">
	<div class="mui-popover-content" style="min-height:220px; padding-right:0;">
    	<p></p>
    	<p>常用<?php  echo $_W['_config']['buytitle'];?>项</p>
        ...
        <ul class="mui-table-view mui-form-btn js-add-form" data-type="diy">
            ...
            <li class="mui-table-view-cell" data-type="9"><a href="javascript:;">+多行文本</a></li>
            ...
            <li class="mui-table-view-cell" data-type="7"><a href="javascript:;">+时间类型</a></li>
            <li class="mui-table-view-cell" data-type="11"><a href="javascript:;">+日期类型</a></li>
            ...
            <li class="mui-table-view-cell" data-type="12"><a href="javascript:;">+视频类型</a></li>
            <li class="mui-table-view-cell" data-type="8"><a href="javascript:;">+地区城市</a></li>
            <li class="mui-table-view-cell" data-type="10"><a href="javascript:;">+手机类型</a></li>
        </ul>
    </div>
    <ul class="mui-table-view">
...
<div id="act_setting" class="mui-popover mui-popover-left" >
	<div class="mui-popover-header"><?php  echo $_W['_config']['buytitle'];?>设置<a href="javascript:;" class="mui-pull-right mui-popover-close js-popover-close" data-popover="act_setting"><me class="mui-icon mui-icon-closeempty"></me></a></div>
	<div class="mui-popover-content">
        <div class="mui-scroll-wrapper-ext">
            <div class="mui-scroll-ext">
                <div class="mui-content-padded">
                    <p class="mui-pl5">设置用户<?php  echo $_W['_config']['buytitle'];?>时填写的表单</p>
                </div>
                <div id="forms" class="mui-input-group group-list">
                    <div class="mui-input-row">
                        <label class="js-form-edit" data-id="realname" data-popover="form_edit"><?php echo empty($sysform['realname']['title'])?'姓名':$sysform['realname']['title']?></label>
                    ...
                    <div class="mui-input-row">
                        <label class="js-form-edit" data-id="mobile" data-popover="form_edit"><?php echo empty($sysform['mobile']['title'])?'手机':$sysform['mobile']['title']?></label>
                    ...
                    <?php (!empty($this) && $this instanceof WeModuleSite || 0) ? (include fx_template('activity/form', TEMPLATE_INCLUDEPATH)) : (include fx_template('activity/form', TEMPLATE_INCLUDEPATH));?>
                    <?php  } } ?>
                </div>
                <div class="mui-text-primary" style="background:#fff;overflow:hidden">
                ...
                <div class="mui-content-padded" style="padding-bottom:10px;">
                    <button type="button" class="mui-btn mui-btn-orange mui-btn-block js-save" data-type="form">完成 · 返回</button>
...
<div id="form_edit" class="mui-popover mui-popover-left mui-popover-sub">
	<div class="mui-popover-header"><?php  echo $_W['_config']['buytitle'];?>项编辑<a href="javascript:;" class="mui-pull-right mui-popover-close js-popover-sub-close" data-popover="form_edit"><me class="mui-icon mui-icon-closeempty"></me></a></div>
    <div class="mui-popover-content">
        <div class="mui-scroll-wrapper">
            <div class="mui-scroll">
            	<div class="mui-input-group">
                    ...
                    <div class="mui-input-row js-desc">
                    	<label>描述</label>
                        <input name="formdesc" value="" type="text" placeholder="不填写默认标题名称">
                    </div>
                </div>
            	<div class="mui-input-group group-child js-child" style="display:none;"></div>
                <div class="mui-content-padded" style="padding-bottom:10px;">
                    <button type="button" class="mui-btn mui-btn-orange mui-btn-block js-save" data-type="formEdit">下一步</button>
                </div>
            </div>
		</div>
    </div>
...
require(['../../../<?php echo FX_BASE;?>app/resource/js/app/Sortable.js'], function (){
...
if ($('#forms .form_item').length > 0){
	$('.js-setting').find("span.mui-badge").text('已设置<?php  echo $_W['_config']['buytitle'];?>填写项');
}else{
	$('.js-setting').find("span.mui-badge").text('姓名、手机号');
}
//表单编辑触发
$("#forms").delegate('.js-form-edit','tap',function(e) {
	formId = $(this).data('id');
	var form_title = $('input[name="form_title['+formId+']"]').val(),
		placeholder = $('input[name="form_title['+formId+']"]').attr('placeholder'),
		form_desc = $('input[name="form_description['+formId+']"]').val();
...
$('.js-save').on('click', function(e) {
	...
	if (type == 'form'){//保存所有form数据
		...
		mui("#act_setting").popover('toggle');history.back(-1);
	}else if(type == 'formEdit'){//表单编辑保存
		var form = 'input[name="form_title['+formId+']"]',
			form_title = $('#form_edit').find('input[name="formtitle"]').val(),
			placeholder = $('#form_edit').find('input[name="formtitle"]').attr('placeholder'),
			form_description = $('#form_edit').find('input[name="formdesc"]').val();
		if (formId=='realname'|| formId=='mobile'){
			form = 'input[name="activity[form]['+formId+'][title]"]';
		}
		$(form).val($.trim(form_title));
		$(form).next().val(form_description);
		...
		if (specId=='default'){
			...
			$('input[name="activity[costprice]"]').val($('.js-spec-val').eq(2).val());
		}else{
