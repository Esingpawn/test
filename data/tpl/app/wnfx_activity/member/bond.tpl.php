...
<script type="text/javascript" src="<?php echo FX_URL;?>app/resource/js/jquery.cookie.js"></script>
<style type="text/css">
...
<?php  if(empty($_W['member']['mobile']) || $actype=='change') { ?>
	<div class="avatar mui-text-center">
		<img src="<?php  echo tomedia($_W['fans']['avatar'])?>" class="mui-img-circle">
        <p class="mui-text-orange mui-big"><?php  echo $_W['fans']['nickname'];?></p>
        <?php  if($actype=='project') { ?>
...
<?php  } else { ?>
    <div class="mui-text-center">
        ...
        <div class="mui-b">你的手机号:<?php  echo $_W['member']['mobile'];?></div>
    </div>
    <div class="mui-content-padded">
...
function WxLogin(form) {
	if (!form['username'].value) {
		...
		return false;
		 ...
		 if (form['username'].value == "<?php  echo $_W['member']['mobile'];?>"){
		...
		if(issend){ 
			...
			if (phoneObj.val() != ""){  
				...
				if (!util.mobile(phoneVal)){
					...
					if (phoneVal == "<?php  echo $_W['member']['mobile'];?>"){
						phoneObj.val('');
						util.alert('当前手机已绑定,无需重复绑定', ' ', function() {
							phoneObj.focus(); 
						}); 
